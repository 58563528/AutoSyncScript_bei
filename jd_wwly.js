...
[task_local]
#汪汪乐园
10 9 * * * jd_crazy_joy.js, tag=汪汪乐园, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_crazy_joy.png, enabled=true
...
async function entryWwly() {
    const body={"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}
    return new Promise(async resolve => {
        $.get(taskUrl('joyList', JSON.stringify(body)), async (err, resp, data) => {
            updateCookie(resp)
            try {
                if (err) {
...
async function doTasks() {
    ...
    for (let j = 0; j < $.taskList.length; ++j) {
        ...
        for (let i = task.taskDoTimes; i < task.taskLimitTimes; ++i) {
...
function getTaskInfo() {
    let body = {"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}
    return new Promise(async resolve => {
        $.post(taskUrl('apTaskList', JSON.stringify(body)), async (err, resp, data) => {
            updateCookie(resp)
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
...
function doTask(taskId) {
    let body = {"taskType":"SIGN","taskId":taskId,"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}
    return new Promise(async resolve => {
        $.get(taskUrl('apDoTask', JSON.stringify(body)), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function taskUrl(functionId, body = '') {
    let t = getTs()
    return {
        url: `${JD_API_HOST}?appid=activities_platform&functionId=${functionId}&body=${escape(body)}&_t=${t}`,
        headers: {
            'Cookie': cookie,
            'Host': 'api.m.jd.com',
            ...
            'Referer': 'https://joypark.jd.com/?activityId=LsQNxL7iWDlXUs6cFl-AAg&lng=0.000000&lat=0.000000&sid=340a9ee19cab9e2e874b0752988cc8bw&un_area=19_1601_50258_50374',
